import Head from "next/head";
import Image from "next/image";
import { Button, Col, Container, Row } from "react-bootstrap";
import about1 from "../public/about5.jpg";
import laptopwork from "../public/laptopwork.svg";
import about2 from "../public/craft.svg";
import about3 from "../public/eager.svg";
import about5 from "../public/joy.svg";
import ola from "../public/ola.png";
import chris from "../public/chris.png";
import piper from "../public/piper.png";
import Timi from "../public/timi2b.png";
import mars from "../public/mars.png";
import rhod from "../public/rhod.png";
import ds from "../public/ds1.png";
import sec from "../public/sec.webp";
import de from "../public/de.png";
import may from "../public/may.png";
import about4 from "../public/excel.svg";
import femaleavatar from "../public/female.svg";
import maleavatar from "../public/male.svg";
import Contact from "../components/contact";
import Aos from "aos";
import React from "react";
import "aos/dist/aos.css";
import Testimonials from "../components/testimonials";
import Link from "next/link";

const About = () => {
  React.useEffect(() => {
    Aos.init({ duration: 800 });
  }, []);
  return (
    <>
      <Head>
        
        <title> About us - Stackciti Technologies</title>
        <meta
          name="description"
          content=" We are a digital agency with a taste for outstanding products"
        />
        <link rel="icon" href="/favicon.ico" />

        <meta
          name="keywords"
          content="Stackciti,  We are a digital agency with a taste for outstanding products best chicago web design agency
best web design agency toronto
ecommerce web design agency london
best chicago web design agency
best web design agency toronto
montana web design agency
element 8 web design agency
best web design digital agency
bespoke web design agency
deep digital web design agency wordpress theme nulled
brisbane web design agency
best web design agency in india
web design agency durham
web design creative agency
web design agency china
agency web design services
fitness web design agency
vermont web design agency
web design agency in abuja
web design agency colorado
tokyo web design agency
best web design agency australia
interactive web design agency
freelance web design agency
web design agency scotland
brighton web design agency
ottawa web design agency
branding web design agency
web design professional agency
graphic and web design agency
crafton web design agency
glasgow web design agency
marketing agency and web design
digital marketing and web design agency
web design agency indonesia
the travel web design agency
web design agency milwaukee
creative web design agency london
web design agency in hyderabad
web design agency in san jose
web design agency in chennai
top 10 web design agency
web design and build agency
best web design agency los angeles
web design agency lancaster pa
web design template for travel agency
web design agency in pune
the web design agency
best web design agency in usa
web design agency uae
design web agency
edinburgh web design agency
web design agency in melbourne
web design agency wellington
web design agency for law firms
web design agency brighton
best web design agency new york
best web design agency in london
restaurant web design agency
web design agency in nigeria
web design company in usa
web design in usa
usa web design company
usa web design
web design companies in usa
web design usa
web design companies usa
web design company usa
usa web design companies
web design agency in usa
web design agencies in usa
web design agency usa
top web design companies in usa
top web design company in usa
top web design companies usa
web design services usa
web design services in usa
web design service usa
best web design companies in usa
best web design company usa
best web design company in usa
web design and development companies in usa
web design and development company usa
web design and development company in usa
web design awards usa
web design service in usa
web design and development in usa
web design development company usa
web design outsourcing companies in usa
ecommerce web design usa
usa best web design company
web design prices in usa
web design and development services usa
best web design agency in usa
web design jobs in usa
web design services company in usa
web design directory usa
web design leads usa
creative and innovative web design companies in spokane washington usa
web design jobs usa
web design packages usa
web design california usa
custom web design usa
web design price in usa
web design studio usa
best web design in usa
best universities for web design and development in usa
responsive web design company usa
best web design and development company in usa
best web design usa
web design prices usa
top 10 web design companies in usa
web design and development usa
responsive web design usa
web design & development company in usa
usa web design company list
custom web design services usa
web design job in usa
web design conferences 2018 usa
web design conferences 2017 usa
web design company directory usa
web design conferences 2019 usa
web design price list in usa
custom web design canada and usa
web design company usa uk
web design & development usa
master's in web design and development in usa
graphic design usa web awards
graphic design usa web design awards
web design franchise usa
freelance web design jobs in usa
master's in web design usa
creative and innovative web design companies in mcallen texas usa
best web design companies usa
web design companies in texas usa
agriculture web design group usa
masters in web design and development in usa
web design and seo company usa
usa best web design
best web design agencies usa
wordpress web design usa
wordpress web design in usa
creative digital web design agencies seattle washington usa
web design business for sale usa
custom web design canada or usa
creative digital web design agencies phoenix arizona usa
web design and development course in usa
web design az usa
web design company in california usa
web design agencies usa
usa best web design agency
usa web design express
web design & development services usa
web design and web development company in usa
custom web design canada vs usa
remote web design jobs usa
masters in web design usa
web development and design in usa
web design company in south dakota sd usa
website designers
website designer
digital marketing agency
web design company
web design companies
web designers near me
web design agency
web designer near me
website design company
web design services
digital agency
google web designer
digital marketing agency near me
website development company
freelance web designers
web agency
web design company near me
companies that build websites
website design company near me
website designer portfolio
web design new york
website building companies
digital marketing agency chicago
best web designers
web designer nyc
web design agency near me
software development company in usa
top web design companies
best web design company
web design services for small business
software services company
digital marketing agency usa
top digital marketing agency
website companies near me
custom web design services
web design services near me
best website design company
web design agency new york
custom web design company
best web design agency
local web design
top 10 website design companies
web designing company in usa
top web designers
huemor
best website designers
top it companies in usa
affordable web design for small business
freelance web designers near me
top web design agency
web design and development agency
web design brands
best web design services
freelance web designer salary
digital marketing agency websites
digital marketing agency website
web design london
digital web agency
jordan crown
best website design companies for small business
custom software development companies in usa
best digital marketing agency websites
web design company names
small business web design near me
digital marketing agency jobs
best website design services
web design and branding agency
black-owned web designers near me
web design services list
best web designers near me
web design manchester
design agency london
top marketing firms in chicago
creative web design agency
web design company in uk
effect web agency
companies that make websites for you
website designers for small business near me
web design names
web development company in uk
top 10 software companies in usa
web development startup
digital marketing agency website template
digital marketing website examples
digital website design
list of web designers
effect website
top digital marketing agencies in chicago
best website company for small business
web development companies list
web developer canada salary
companies that build websites for small businesses
comrade digital marketing agency
free website design for small business
best web design agency in the world
best website designer near me
web design company london
best web designers in the world
website designer jobs near me
web design services for small business near me
huemor designs
best website design company in usa
web development company in canada
freelance web designer new york
web designer 23
digital marketing agency web design
best custom website design company
web design company name generator
website design and build
companies that create websites for businesses
top 20 software companies in usa
best web developers in the world
it companies in europe
new york web coders
digital marketing agency description
how to start a digital marketing agency
uk web design
web design uk
design web uk
web design in uk
uk web design company
uk web design companies
web design companies uk
web design agency in uk
web design company in uk
uk web design agency
web design company uk
web design agency uk
web design companies in uk
web design agencies uk
web design agencies in uk
web design service uk
web design services uk
web design service in uk
web design course uk
best online web design courses uk
web design and development services uk
uk web design shop
web design software uk
best web design agencies uk
web design companies in london uk
web design northampton uk
web design cambridge uk
web design prices uk
web design company london uk
wix web design uk
web design forum uk
web design contract uk
best web design company uk
web design price list uk
top web design companies uk
web design birmingham uk
web page design uk
remote web design jobs uk
web design course in uk
web design packages uk
freelance web design uk
uk web design prices
web design lancaster uk
web design courses in uk
uk best web design company
web design and development uk
uk web design association
web design norfolk uk
web design masters uk
web design courses online with certificate uk
web design uk salary
web design kent uk
web hosting and design uk
web design west midlands uk
web design freelance jobs uk
responsive web design company uk
web design salary uk
responsive web design uk
web design studio uk
web design directory uk
cost of web design uk
web design degree uk
web design hampshire uk
web design awards uk
web design norwich uk
pro web design uk
uk web design forum
online web design courses uk
wordpress web design uk
web design and hosting uk
uk web design directory
web design brighton uk
web design for small business uk
cheap web design uk
web design course online uk
web design business for sale uk
web design london uk
web design manchester uk
top web design agencies uk
ecommerce web design uk
web design peterborough uk
web design contract template uk
best web design agency in uk
web design university uk
free online web design courses uk
web design worcester uk
web design shrewsbury uk
pay monthly web design uk
web design for charities uk
red web design uk
freelance web design jobs uk
creative web design uk
ecommerce web design company uk
web design companies birmingham uk
web design southampton uk
best web design company in uk
web design lincoln uk
best web design uk
web design and development company in uk
bespoke web design uk
top uk web design companies
web design co uk
web design in london uk
uk web design jobs
web design portsmouth uk
web design price uk
web design companies uk list
small business web design uk
free web design uk
affordable web design uk
web design cost uk
web design university courses uk
web design costs uk
web design and development course in uk
web design jobs from home uk
web design nottingham uk
web design surrey uk
web design franchise uk
web design bristol uk
web design and development company uk
web design essex uk
free web design courses uk
top 10 uk web design companies
web design online courses uk
top web design companies in uk
best web design companies uk
best web design courses uk
best web design agency uk
web design courses uk
squarespace web design uk
web design courses online uk
online web design course uk
web design jobs uk
web design uk prices
web design york uk
uk web design awards
web design leads uk
web design hourly rate uk
web design magazine uk
web design labs uk reviews
web design bath uk
web design suffolk uk
web design derby uk
white label web design uk
msc in web design and development in uk
kd web design london uk
web design agency manchester uk
web design bolton uk
web design plymouth uk
buy web design leads uk
hummingbird uk web design
setting up a web design business uk
web design labs uk
web design tenders uk
web design cheltenham uk
web design consultant uk
web design rates uk
work from home web design jobs uk
web design quote uk
web design north east uk
web design sic code uk
web design cornwall uk
starting a web design business uk
web design certification uk
web design tamworth uk
web design hereford uk
web design company usa uk
web design oxford uk
universal web design uk
web design and development course uk
seo web design company in uk
masters in web design uk
web design industry statistics uk
web design conferences uk 2019
web design terms and conditions template uk
web design market size uk
web design conferences 2018 uk
custom web design canada vs uk
web design for musicians uk
web design glasgow uk
web design courses in reading uk
web design news uk
custom web design canada or uk
web design company for sale uk
uk web design leads
web design companies in birmingham uk
web design courses uk universities
web design and hosting services uk
web design course birmingham uk
visionary web design uk
custom web design canada versus uk
web design pricing uk
agriculture web design group uk
web design website in uk
responsive web design services uk
top universities for web design uk
to web design service in the uk
web design events uk 2020
2137 web design and development professionals uk
the web design company uk
spider web design uk
web design southport uk
web design consultant salary uk
web design lincolnshire uk
web design packages examples uk
web design for female entrepreneurs uk
sic code uk web design
web design lane uk
web design bootcamp uk
uk churches web design
award winning web design uk
web design conferences 2019 uk
graphic and web design studio uk
cost of web site design uk liverpool
uk web design contract
will web design agency work in uk
sic code for web design uk
woocommerce web design uk
web design outsourcing companies in uk
web design and development courses uk
web design south west uk
wix
logo maker
wix login
free logo maker
website builder
webflow
website designers
free website builder
website builders free
create a website
digital marketing agency
best website builder
web design company
web design companies
wix logo maker
wix pricing
godaddy website builder
website designer near me
logo design companies
wix website builder
google website builder
web design agency
wix logo
wix templates
wix editor
webflow templates
web design services
wordpress website builder
wordpress free
best free website builder
best website builder for small business
web development companies
is wix free
wordpress vs wix
free website builder and domain
website builder software
webflow login
logo design near me
webflow vs wordpress
website design company near me
web design company near me
web agency
free website builder for small business
best web designers
best web design software
web design agency near me
fiverr website design
best logo design company
website design prices
best web design company
top web design companies
web design services for small business
best web design companies
local web designers near me
local website designers near me
website designer online
best website builder for artists
wix free website builder
website companies near me
average cost of website design for small business
wordpress hosting cost
best website builder software
best website design company
web design services near me
logo near me
website builder software free
website cost calculator
website design cost calculator
web design agency new york
local web design
best web design agency
best website designers
web design company usa
top 10 website design companies
marketing agency london
digital agency website
affordable web design for small business
freelance web design jobs
freelance web designers near me
list of website builders
wordpress pricing table
freelance website designer near me
wix.com dashboard
free website builder for students
web design london
web design brands
web design price list
website price calculator
web design and development agency
best web design services
best website design agency
advertising agency website
offline website builder software
5 page website price
best website design companies for small business
web design agency usa
best website builder for beginners
offline website builder software for windows 10
webflow vs figma
webflow download
web design company names
digital agency london
website cost per month
website estimate
top web design company in usa
top web design companies in usa
web design agency london
digital design agency london
mike kus
website builder software windows
creative web design agency
black-owned web designers near me
website design cost per hour
web design services list
web design services usa
web agency london
digital agency uk
companies that make websites for you
website designers for small business near me
web design names
list of web designers
web design jobs from home
logo and website design near me
website layout design examples
wordpress cost vs wix
companies that build websites for small businesses
free website design for small business
black graphic designers near me
best website builder for freelancers
website layout design tools
wordpress website development cost
best web designers in the world
website designer jobs near me
website estimate template
wordpress website cost estimator
best website designer near me
best web design agency in the world
list of free website builders
most secure website builder
website design and build
wordpress web design london
best website design company in usa
website development cost in usa
website design cost estimate template
website design estimate template
do wordpress plugins cost money
web design company name generator
best custom website design company
creative agency website templates
creative agency website inspiration
agency website examples
companies that create websites for businesses
top 10 web developers in the world
black female web designers
black owned web hosting companies
entry level web design jobs near me
web designer jobs no experience
best website builder for influencers
website builder software without coding
how much does a website cost per month
how to create a website for free on google
how much to charge for website design
how to design a website free
how to design a website layout
how much to charge for a wordpress website 2021
which web design company uk
how much to charge for web design uk
how many web design companies in the uk
"
        />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <section id="about-section" data-aos="fade-up">
        <Container className="about">
          <Row>
            <Col sm>
              <button id="fade-btn">About</button>
              <div>
                <h2>
                  We are a digital agency with a taste for outstanding products.
                </h2>
              </div>
            </Col>
          </Row>
        </Container>
        <br />
        <br />
        <br />
        <br />
        <Container className="containers">
          <Row>
            <Col sm>
              <p id="words">
                Driven by both skill , we work with company change-makers and
                visionaries to create the world’s most exciting brand interfaces
              </p>
            </Col>
            <Col sm>
              <Image src={laptopwork} alt="image" width={500} height={350} />
            </Col>
          </Row>
        </Container>
        <Container className="containers">
          <h4 className="title-n" data-aos="fade-up">
            Our Story.
          </h4>
          <p className="title-text" data-aos="fade-up">
            <strong>Stackciti.com</strong> is a web design and app development
            agency that helps small businesses and individuals get in the face
            of their target audience through the creation of websites that are
            not only visually appealing and user-friendly, but also secure with
            the latest cyber security protocols.
          </p>
          <p className="title-text" data-aos="fade-up">
            <strong>STATISTICS</strong> have shown that an{" "}
            <strong>
              {" "}
              average of 30,000 websites are hacked each day, and this leaves
              these businesses destabilized as important files are lost. Also,
              43% of cyber attacks target small businesses and only 5% of their
              folders and files{" "}
            </strong>{" "}
            are properly protected which spells doom to the {`brands'`} growth.
          </p>
          <p className="title-text" data-aos="fade-up">
            <Image src={sec} alt="image" width={1000} height={600} />
          </p>
          <p className="title-text" data-aos="fade-up">
            <strong>THIS IS WHY</strong> at Stackciti.com, beyond creating a
            website for your business, we care about the performance of your
            websites so that it {`doesn't`} come under attack.{" "}
            <strong>
              We make sure your website is able to withstand any cyber threat so
              that you can focus on growing your business and connecting with
              your customers.
            </strong>
          </p>
          <p className="title-text" data-aos="fade-up">
            Our goal is to see your brand succeed; so we go the extra mile to
            ensure your website is secure just as it is beautiful. We have a
            brilliant team of experienced and creative web developers and
            designers that deliver innovations to advance your business. Our
            priority is to deliver a safe and secure website that you can trust.
          </p>
          <p className="title-text" data-aos="fade-up">
            We look forward to helping you create a perfect digital presence.
          </p>
          <p className="title-text" data-aos="fade-up">
            <Link href="/contact">
              <Button>GET IN TOUCH TODAY</Button>
            </Link>
          </p>
        </Container>
        <Testimonials />
        <Container id="values" className="containers">
          <h2 data-aos="fade-up" className="title-n">
            CORE VALUES
          </h2>
          <br />

          <Row>
            <Col sm>
              <div>
                <Image src={about2} alt="image" width={300} height={300} />
              </div>
              <div>
                <h5 id="title">Craft</h5>
                <h6>Never stop learning</h6>
                <p>
                  Treasure our experience and foster the excitement to master
                  the art of everything you do.
                </p>
              </div>
            </Col>
            <Col sm data-aos="fade-down">
              <div>
                <Image src={about3} alt="image" width={300} height={300} />
              </div>
              <div>
                <h5 id="title">Eagerness</h5>
                <h6>{`Keep asking 'why?'`}</h6>
                <p>
                  Pursue an unstoppable drive to make experiences wholesome and
                  an absolute delight to use.
                </p>
              </div>
            </Col>
          </Row>
          <Row>
            <Col sm data-aos="fade-right">
              <div>
                <Image src={about4} alt="image" width={300} height={300} />
              </div>
              <div>
                <h5 id="title">Excellence</h5>
                <h6>Be better & do better</h6>
                <p>
                  Go above and beyond quality and strive for excellence in every
                  aspect of the process and deliverable.
                </p>
              </div>
            </Col>
            <Col sm data-aos="fade-up">
              <div>
                <Image src={about5} alt="image" width={300} height={300} />
              </div>
              <div>
                <h5 id="title">Joy</h5>
                <h6>{`Let’s make something!`}</h6>
                <p>
                  We thrive when the work doesn’t feel like work and results
                  seemingly come without effort.
                </p>
              </div>
            </Col>
          </Row>
        </Container>
        <br />
        <Container className="team">
          <h3>Meet the team</h3>
          <p>
            Our team is jam packed with creative, smart and bubbly people. We
            inspire each other to improve daily, and be the best and happiest we
            can be.
          </p>
          <Row>
            <Col sm>
              <Image src={ola} alt="image" width={100} height={100} />
              <h1>Sotunde Olamide</h1>
              <h4>Founder,CEO</h4>
            </Col>
            <Col sm>
              <Image src={mars} alt="image" width={100} height={100} />
              <h1>Chris Mars</h1>
              <h4>Co-Founder & CTO </h4>
            </Col>
            <Col sm>
              <Image src={rhod} alt="image" width={100} height={100} />
              <h1>Otun Rhodiat</h1>
              <h4>Product manager</h4>
            </Col>
            <Col sm>
              <Image src={de} alt="image" width={100} height={100} />
              <h1>Bamidele Imisioluwa</h1>
              <h4>Chief marketing officer</h4>
            </Col>
          </Row>
          <Row>
            <Col sm>
              <Image src={piper} alt="image" width={100} height={100} />
              <h1>Sophie piper</h1>
              <h4>Blockchain developer</h4>
            </Col>
            <Col sm>
              <Image src={chris} alt="image" width={100} height={100} />
              <h1>Chris Luhrs</h1>
              <h4>Product design lead</h4>
            </Col>
            <Col sm>
              <Image src={may} alt="image" width={100} height={100} />
              <h1>Adewale Mayowa</h1>
              <h4>Product designer</h4>
            </Col>
            <Col sm>
              <Image src={Timi} alt="image" width={100} height={100} />
              <h1>Timi Oyelumade</h1>
              <h4>Graphic designer</h4>
            </Col>
          </Row>
          <Row>
            <Col sm>
              <Image src={ds} alt="image" width={100} height={100} />
              <h1>Daniel Orobode</h1>
              <h4>Sales manager</h4>
            </Col>
          </Row>
        </Container>

        <Contact />
      </section>
    </>
  );
};

export default About;
